package org.archicontribs.modelrepository.process;

import org.eclipse.osgi.util.NLS;

public class Messages extends NLS {

    private static final String BUNDLE_NAME = "org.archicontribs.modelrepository.process.messages"; //$NON-NLS-1$

    public static String CommitModelProcess_0;

    public static String CommitModelProcess_1;

    public static String CommitModelProcess_2;

    public static String PushModelProcess_0;

    public static String PushModelProcess_1;

    public static String PushModelProcess_2;

    public static String PushModelProcess_3;

    public static String PushModelProcess_4;

    public static String RefreshModelProcess_0;

    public static String RefreshModelProcess_1;

    public static String RefreshModelProcess_2;

    public static String RefreshModelProcess_3;

    public static String RefreshModelProcess_4;

    public static String RefreshModelProcess_5;

    public static String RefreshModelProcess_6;

    public static String RefreshModelProcess_7;

    public static String RefreshModelProcess_8;

    public static String RefreshModelProcess_9;

    public static String RefreshModelProcess_10;

    public static String RefreshModelProcess_11;

    public static String RefreshModelProcess_12;

    public static String RefreshModelProcess_13;

    public static String RefreshModelProcess_14;

    public static String RefreshModelProcess_15;

    public static String RepositoryModelProcess_0;

    public static String RepositoryModelProcess_1;

    public static String RepositoryModelProcess_2;

    public static String RepositoryModelProcess_3;

    public static String RepositoryModelProcess_4;

    public static String RepositoryModelProcess_5;

    public static String RepositoryModelProcess_6;

    public static String RepositoryModelProcess_7;

    public static String RepositoryModelProcess_8;

    public static String RepositoryModelProcess_9;

    static {
        // initialize resource bundle
        NLS.initializeMessages(BUNDLE_NAME, Messages.class);
    }

    private Messages() {
    }
}
